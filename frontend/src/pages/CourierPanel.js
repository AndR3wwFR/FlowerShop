...
const CourierPanel = () => {
  ...
  const loadOrders = async () => {
    try {
      const response = await api.get('/orders/for-delivery');
      console.log('Заказы для курьера:', response.data);
  ...
  const startDelivery = async (orderId) => {
    setLoading(true);
    try {
      await api.put(`/orders/${orderId}/start-delivery`);
  ...
  const getStatusText = (status) => {
    if (status === 2 || status === 'Ready') return { text: '✅ Готов к доставке', color: '#4caf50', action: 'deliver' };
    if (status === 3 || status === 'Delivering') return { text: '🚚 В пути', color: '#2196f3', action: 'complete' };
  ...
  const styles = {
    ...
    stats: { background: '#e3f2fd', padding: '15px 20px', borderRadius: '12px', marginBottom: '25px', display: 'flex', justifyContent: 'space-between' },
    ...
    orderInfo: { display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '20px' },
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>🚚 Доставка заказов</h1>

      <div style={styles.stats}>
        <span>🚛 Курьер: {user?.fullName}</span>
        <span>📦 Заказов на доставку: {orders.length}</span>
      </div>
            ...
            return (
              <div key={order.id} style={styles.orderCard}>
                ...
                <div style={styles.itemsList}>
                  <div style={{ fontWeight: 'bold', marginBottom: '10px' }}>🌸 СОСТАВ ЗАКАЗА:</div>
                ...
                <div style={styles.buttonGroup}>
                  ...
                  {/* Кнопка для статуса "Готов к доставке" (2 или 'Ready') */}
                  {(order.status === 2 || order.status === 'Ready') && (
                    <button onClick={() => startDelivery(order.id)} disabled={loading} style={styles.primaryButton}>
                      {loading ? 'Загрузка...' : '🚚 Начать доставку'}
                    </button>
                  )}
                  
                  {/* Кнопка для статуса "В пути" (3 или 'Delivering') */}
                  {(order.status === 3 || order.status === 'Delivering') && (
                    <button onClick={() => completeDelivery(order.id)} disabled={loading} style={styles.successButton}>
                      {loading ? 'Загрузка...' : '✅ Доставлен'}
                    </button>
                  )}
                </div>
              </div>
